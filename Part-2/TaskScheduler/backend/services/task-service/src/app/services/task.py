...
class TaskService:
    def __init__(self, repository: TaskRepository, event_bus: TaskEventBus) -> None:
    ...
    async def update_task_status(self, task_id: int, status_value: TaskStatus) -> Task:
        ...
        updated = await self.repository.update_status(task, status_value)
        await self.event_bus.publish(
            TaskEvent(
                event_type=TaskEventType.STATUS_CHANGED,
                task_id=task_id,
                user_id=task.user_id,
                metadata={"new_status": status_value},
            )
        )
        return updated
